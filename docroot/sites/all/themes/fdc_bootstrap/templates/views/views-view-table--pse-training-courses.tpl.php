 ...
 */
global $this_is_not_first_events_table, $this_is_not_first_events_table_header_item_no; // output headers, just once
    ...
    <thead>
      <tr>
        <?php foreach ($header as $field => $label): ?>
          ...
          <?php if($field != 'edit_url' || user_access('bypass node access')) { ?>
    ...
    <?php foreach ($rows as $row_count => $row): ?>
      <tr <?php if ($row_classes[$row_count]) { print 'class="' . implode(' ', $row_classes[$row_count]) .'"';  } ?>>
        <?php foreach ($row as $field => $content): ?>
          <?php //dsm($field); ?>
          <?php if ($content == 'empty' || $field == 'edit_url' && strpos($content, 'empty') !== false) { ?>
          <?php } else { ?>
